...
function Install-ADTDeployment
{
    ...
    Uninstall-ADTApplication -Name "VisualStudioCode" -ApplicationType 'MSI'
    Uninstall-ADTApplication -Name "Microsoft Visual Studio Code" -ApplicationType 'MSI'
    Uninstall-ADTApplication -Name "Microsoft Visual Studio Code" -ApplicationType 'EXE' -ArgumentList "/VERYSILENT /NORESTART /SUPPRESSMSGBOXES /LOG=""$((Get-ADTConfig).Toolkit.LogPath)\$($adtSession.InstallName)_EXE_Uninstall.log"""



    ################################################################# Uninstallation of User installed Apps #######################################################################
    $LogU = Get-ADTLoggedOnUser
    $currentUsername = $LogU.Username
    ...
    if ($uninstallString) {
        $uninstallCommand = $uninstallString.UninstallString + " /VERYSILENT /NORESTART /SUPPRESSMSGBOXES"
        Write-ADTLogEntry $uninstallCommand
        ...
        Write-ADTLogEntry -Message "Uninstalled User installed VSCode - $DisplayVer"
        Write-ADTLogEntry -Message "Uninstall Command used - $uninstallCommand"
        Remove-ADTRegistryKey -Key "$Uninstallhive"
        Remove-ADTFolder -Path "C:\Users\$currentUsername\AppData\Local\Programs\Microsoft VS Code" 
        }Catch{
        Write-ADTLogEntry -Message "Uninstallation of user installed VSCode failed: $_ "
        }
    }else { Write-ADTLogEntry -Message "No Standard User installed VSCode app found." }
    } else {
        Write-ADTLogEntry -Message "No Standard User installed VSCode app found."
    }
    ...
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\317_Microsoft_VisualStudioCode_1.76.2_EN_R1'
    ...
    Start-ADTProcess -FilePath "$($adtSession.DirFiles)\$($adtSession.InstallerName)" -ArgumentList "/VERYSILENT /NORESTART /SUPPRESSMSGBOXES /MERGETASKS=!runcode /LOG=""$((Get-ADTConfig).Toolkit.LogPath)\$($adtSession.InstallName)_EXE_Install.log""" -WindowStyle "Hidden"
    ...
    $ARPs = Get-ADTApplication -Name '*Visual Studio Code*'
    ...
    Copy-ADTFileToUserProfiles -Path "$($adtSession.DirFiles)\settings.json" -Destination "AppData\Roaming\Code\" -Recurse


    $ARPs = Get-ADTApplication -Name '*Visual Studio Code*' -ApplicationType 'EXE'    
...
function Uninstall-ADTDeployment
{
    ...
    Show-ADTInstallationWelcome -CloseProcesses @{Name ="Code"} -Silent
    ...
    Uninstall-ADTApplication -Name "*Visual Studio Code*" -ApplicationType 'EXE' -ArgumentList "/VERYSILENT /NORESTART /SUPPRESSMSGBOXES /LOG=""$((Get-ADTConfig).Toolkit.LogPath)\$($adtSession.InstallName)_EXE_Uninstall.log"""
    ...
    Remove-ADTFolder -Path "$envSystemDrive\Users\Default\AppData\Roaming\Code"    

    $ARPs = Get-ADTApplication -Name 'Microsoft Visual Studio Code'
    if (Compare-Version $($adtSession.AppVersion) $ARPs.DisplayVersion "eq") {}
