...
function Install-ADTDeployment
{
    ...
    #========================================================================
    ##Previous or laterst VC++ x86 if present:
    #========================================================================
    ...
    $ARP = Get-ADTApplication -Name "Microsoft Visual C++ * Redistributable (x86)*" | Where-Object { $_.DisplayVersion -like '14.*' -and $_.SystemComponent -ne "1" }
    $InstVer = $ARP.DisplayVersion
    If([version]$InstVer -lt [version]$($adtSession.AppVersion))
    {
        Write-ADTLogEntry -Message "Lower version ($InstVer) of Microsoft Visual C++ Reditributable is installed. Installing compliant version..."
        
        Start-ADTProcess -FilePath "$($adtSession.DirFiles)\$($adtSession.InstallerName)" -ArgumentList "/install /passive /quiet /norestart" -WindowStyle 'Hidden' -IgnoreExitCodes '3010,1641'
    }
    else
    {
        Write-ADTLogEntry -Message "Higher or same version ($InstVer) of the application is installed and it's compliant. Skipping the installation..."
    }
    ...
    $ARPs = Get-ADTApplication -Name "Microsoft Visual C++ * Redistributable (x86)*" | Where-Object { $_.DisplayVersion -like '14.*' -and $_.SystemComponent -ne "1" }
    ForEach ($ARP in $ARPs)
    {
        If ($($ARP.PSPath)) 
        {
