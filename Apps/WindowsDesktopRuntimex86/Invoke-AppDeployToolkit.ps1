...
function Install-ADTDeployment
{
    ...
    ## <Perform Pre-Installation tasks here>
      ...
      $ARPs = Get-ADTApplication -Name 'Microsoft Windows Desktop Runtime*(x86)' | Where-Object { $_.SystemComponent -like "False" }
      foreach ($app in $ARPs){
      If([version]$app.DisplayVersion -lt [version]$($adtSession.AppVersion)){
    ...
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AllianzPackages\1030_Microsoft_WindowsDesktopRuntime_8.0.12_PKG_R1' -ErrorAction SilentlyContinue

    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AllianzPackages\1030_Microsoft_WindowsDesktopRuntime_8.0.12_PKG_R2' -ErrorAction SilentlyContinue

    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AllianzPackages\1946_Microsoft_WindowsDesktopRuntime_8.0.18_PKG_R1' -ErrorAction SilentlyContinue

    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AllianzPackages\2840_Microsoft_WindowsDesktopRuntime_8.0.21_PKG_R1' -ErrorAction SilentlyContinue
    ...
    $ARP = Get-ADTApplication -Name 'Microsoft Windows Desktop Runtime*(x86)' | Where-Object { $_.SystemComponent -like "False" }
    If(([version]$ARP.DisplayVersion -ne [version]$($adtSession.AppVersion)) -or ($null -eq $ARP)){
        Start-ADTProcess -FilePath "$($adtSession.DirFiles)\$($adtSession.InstallerName)" -ArgumentList "/S" -WindowStyle "Hidden"    
        }
    else
    {
    ...
    $ARPs = Get-ADTApplication -Name 'Microsoft Windows Desktop Runtime*(x86)'
    ...
    }else{Write-ADTLogEntry -Message "Microsoft Windows Desktop Runtime (x86) app not found, Skipping Branding creation"}
...
function Uninstall-ADTDeployment
{
    ...
    #Show-ADTInstallationWelcome -CloseProcesses @{Name =""} -Silent
    ...
    ## <Perform Uninstallation tasks here>
       ...
       Uninstall-ADTApplication -FilterScript {$_.DisplayName -like "Microsoft Windows Desktop Runtime*(x86)"  -and $_.DisplayVersion -eq $($adtSession.AppVersion)} -ArgumentList "/uninstall /quiet"
    ...
    if (Compare-Version $($adtSession.AppVersion) $ARPs.DisplayVersion "eq"){Write-ADTLogEntry -Message "Microsoft Windows Desktop Runtime (x86) app found, Skipping Branding deletion"}
