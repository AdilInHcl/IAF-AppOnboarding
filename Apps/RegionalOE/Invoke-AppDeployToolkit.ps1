...
$adtSession = @{
    ...
    InstallName = "1234_Test_PSADT_4.1.5_PKG_R1"
    InstallTitle = " "
...
function Install-ADTDeployment
{
    ...
    If ($envComputerName.Substring(0,3) -eq 'AMC') {$Platform = 'AMC'}
    Else {
        $CRCPlatform = [System.Environment]::GetEnvironmentVariable('CRC_Platform', 'Machine')
        If ($CRCPlatform -eq 'AVC') {
            $Platform = 'AVC'

            $CRCDC = [System.Environment]::GetEnvironmentVariable('CRC_DC', 'Machine') 
            If ($CRCDC -in @('A1','A2')) {$DC = 'A1'}
            ElseIf ($CRCDC -in @('E1','E2')) {$DC = 'E1'}

            $CRCOE = [System.Environment]::GetEnvironmentVariable('CRC_OE', 'Machine') 
            If ($CRCOE -eq 'AZT') {$OE = 'AZT'}
        }
    }

    Invoke-ADTAllUsersRegistryAction {
        Set-ADTRegistryKey -Key 'HKCU:\Software\Dummy Package to test regional Config\Settings' -Name 'Platform' -Value "$Platform" -Type String -SID $_.SID
        Set-ADTRegistryKey -Key 'HKCU:\Software\Dummy Package to test regional Config\Settings' -Name 'DC' -Value "$DC" -Type String -SID $_.SID
        Set-ADTRegistryKey -Key 'HKCU:\Software\Dummy Package to test regional Config\Settings' -Name 'OE' -Value "$OE" -Type String -SID $_.SID
    }
...
function Uninstall-ADTDeployment
{
    ...
    Invoke-ADTAllUsersRegistryAction {
        Remove-ADTRegistryKey -Key 'HKCU:\Software\Dummy Package to test regional Config' -SID $_.SID -Recurse
    }
