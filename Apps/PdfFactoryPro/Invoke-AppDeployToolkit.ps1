...
function Install-ADTDeployment
{
    ...
    #Cleanup
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\458_FinePrintSoftware_pdfFactoryPro_8.41_PKG_R1' 
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\458_FinePrintSoftware_pdfFactoryPro_8.41_PKG_R2' 
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\458_Test_FPS_PDFFactoryPro_8.41_PKG_R1' 
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\778_FinePrint_pdfFactoryPro_9.0_PKG_R1'
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\778_FinePrint_pdfFactoryPro_9.0_PKG_R2'
    Remove-ADTFile -Path "$envCommonDesktop\pdfFactory Pro.lnk"
    ...
    Start-ADTProcess -FilePath "$($adtSession.DirFiles)\$($adtSession.InstallerName)" -ArgumentList "/install /quiet /noini" -WindowStyle "Hidden"
    ...
    If($Platform -eq "RAPPS"){
       ...
       Write-ADTLogEntry -Message "Applying user configuration for AMC/AVCC."
        ...
        Remove-ADTFile -Path "$envSystemdrive\Users\$LUname\OneDrive - Allianz\Desktop\pdfFactory Pro.lnk"
        Remove-ADTFolder -Path "$envSystemdrive\Users\$LUname\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\pdfFactory Pro"
		
        #License activation
        Invoke-ADTAllUsersRegistryAction {
            Set-ADTRegistryKey -Key 'HKCU\Software\FinePrint Software\pdfFactory9' -Name 'RegName' -Value 'Allianz Technology' -Type String -SID $_.SID
            Set-ADTRegistryKey -Key 'HKCU\Software\FinePrint Software\pdfFactory9' -Name 'RegNum' -Value 'H5W3F-UXXBS-KFSUR' -Type String -SID $_.SID
            Set-ADTRegistryKey -Key 'HKCU\Software\FinePrint Software\pdfFactory9\CustomSettings' -Name 'NoDesktopIcon' -Value '1' -Type Dword -SID $_.SID
        }
        
		
    }
    #Hiding Remove\Change button in AddRemovePrograms
    $ARPs = Get-ADTApplication -Name 'pdfFactory Pro'
    ...
    }else{Write-ADTLogEntry -Message "PDF Factory Pro app not found, Skipping Branding creation"}
...
function Uninstall-ADTDeployment
{
    ...
    #Cleanup
    Invoke-ADTAllUsersRegistryAction {
        Remove-ADTRegistryKey -Key 'HKCU\Software\FinePrint Software\pdfFactory9' -Name 'RegName' -SID $_.SID
        Remove-ADTRegistryKey -Key 'HKCU\Software\FinePrint Software\pdfFactory9' -Name 'RegNum' -SID $_.SID
    }

    $ObjectPath = "$envProgramFiles\FinePrint Software"
    Remove-ADTFolder -Path "$ObjectPath\pdfFactory9"
    ...
    $ARPs = Get-ADTApplication -Name 'pdfFactory Pro'
    if (Compare-Version $($adtSession.AppVersion) $ARPs.DisplayVersion "eq") {Write-ADTLogEntry -Message "PDF Factory Pro app found, Skipping Branding deletion"}
