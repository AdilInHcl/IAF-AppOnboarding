...
$adtSession = @{
    ...
    AppProcessesToClose = @('Putty')  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    Uninstall-ADTApplication -Name "PuTTY" -ApplicationType 'MSI'

    #Cleanup
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\478_Simon_PuTTY_0.80_PKG_R1'
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\554_SimonTatham_PuTTY_0.81_PKG_R1'
    ...
    Remove-ADTFile -Path "$envCommonDesktop\PuTTY (64-bit).lnk" 
    Remove-ADTFile -Path "$envCommonStartMenuPrograms\PuTTY (64-bit)\PuTTY Manual.lnk"
    Remove-ADTFile -Path "$envCommonStartMenuPrograms\PuTTY (64-bit)\PuTTY Web Site.lnk"
   ...
   $ARPs = Get-ADTApplication -Name 'PuTTY*'
    ...
    }else{Write-ADTLogEntry -Message "Putty app not found, Skipping Branding creation"}
...
function Uninstall-ADTDeployment
{
    ...
    Show-ADTInstallationWelcome -CloseProcesses @{Name ="Putty"} -Silent
    ...
    $ARPs = Get-ADTApplication -Name 'PuTTY*'
    if (Compare-Version $($adtSession.AppVersion) $ARPs.DisplayVersion "eq") {Write-ADTLogEntry -Message "Putty app found, Skipping Branding deletion"}
