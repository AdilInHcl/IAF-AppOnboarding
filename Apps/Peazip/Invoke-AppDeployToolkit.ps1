...
$adtSession = @{
    ...
    AppProcessesToClose = @('PeaZip')  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    Uninstall-ADTApplication -Name "*PeaZip*" -ApplicationType 'EXE' -ArgumentList "/SILENT"
    ...
    Start-ADTProcess -FilePath "$($adtSession.DirFiles)\$($adtSession.InstallerName)" -ArgumentList "/SILENT /NORESTART /SUPPRESSMSGBOXES" -WindowStyle "Hidden"
    Start-Sleep -Seconds 10
    ...
    Remove-ADTFile -Path "$envProgramData\Microsoft\Windows\Start Menu\Programs\Uninstall PeaZip.lnk"
    #Remove-ADTFile -Path "$envProgramData\Microsoft\Windows\Start Menu\Programs\Configure PeaZip.lnk"

    #Hiding Remove\Change button in AddRemovePrograms
    $ARPs = Get-ADTApplication -Name '*PeaZip*'
