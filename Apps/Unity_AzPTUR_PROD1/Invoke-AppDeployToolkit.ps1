...
function Install-ADTDeployment
{
    ...
    if ($oldbrandingKeys){
       foreach($keys in $oldbrandingKeys){
            Remove-ADTRegistryKey -Key $_.PSPath -Recurse -ErrorAction SilentlyContinue
            }   
       }
    ...
    try{
        ...
        Expand-Archive -Path "$($adtSession.DirFiles)\$Zipfile" -DestinationPath "$envProgramFilesX86\Unity_$($UnityEnv)" -Force
    }Catch{
    ...
    if (Test-Path "$envCommonStartMenuPrograms\Unity_$($UnityEnv)"){Write-ADTLogEntry -Message "Shortcut already Present, Shortcut creation skipped"}else{
        $fileinfo = Get-Item -Path "$envProgramFilesX86\Unity_$($UnityEnv)\Unity.exe"     
        ...
        New-ADTShortcut -Path "$envCommonStartMenuPrograms\Unity_$($UnityEnv)\$($description) $($UnityEnv).lnk" -TargetPath "$envProgramFilesX86\Unity_$($UnityEnv)\Unity.exe" -IconLocation "$envProgramFilesX86\Unity_$($UnityEnv)\Unity.exe" -Description "Unity_$($UnityEnv)_$($adtSession.AppVersion)" -WorkingDirectory "$envProgramFilesX86\Unity_$($UnityEnv)\"    
    }
    ...
    if (Test-Path "$envProgramFilesX86\Unity_$($UnityEnv)\Unity.exe") 
    {
        ...
        Set-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$($adtSession.AppName)" -Name 'DisplayIcon' -Value "$envProgramFilesX86\Unity_$($UnityEnv)\Unity.ico" -Type String
