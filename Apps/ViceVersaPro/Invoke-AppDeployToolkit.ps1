...
function Install-ADTDeployment
{
    ...
    Uninstall-ADTApplication -Name "ViceVersa Pro*" -ApplicationType 'EXE'

    #Cleanup
  # Commented out for testing, but should be enabled for production to ensure clean uninstall before install.
  #  Remove-ADTFolder -Path "$envProgramData\AZCopy"
  #  Remove-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\0325_Microsoft_AzureCopy_10.19_EN_R01" -Recurse
  #  Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AllianzPackages\1388_Microsoft_AzureCopy_10.29.0_PKG_R1' -Recurse
    ...
    #Main Installation
    $AppSetupFileName = Get-ChildItem -Path $($adtSession.DirFiles) -File -Recurse | Where-Object { $PSItem.Extension -like ".exe"} | Sort-Object -Property "LastModified" -Descending | Select-Object -First 1
    Copy-ADTFile -Path "$($AppSetupFileName.FullName)" -Destination "$envProgramData\ViceVersaPro\" -Recurse
    ...
    Set-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\AZCopy" -Name "Publisher" -Value "$($adtSession.AppVendor)" -Type String
    Set-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\AZCopy" -Name "DisplayName" -Value "$($adtSession.AppName)" -Type String
    Set-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\AZCopy" -Name "DisplayVersion" -Value "$($adtSession.AppVersion)" -Type String
    Set-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\AZCopy" -Name "UninstallString" -Value "C:\ProgramData\AZCopy.exe" -Type String
    Set-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\AZCopy" -Name "NoModify" -Value "1" -Type DWORD
    Set-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\AZCopy" -Name "NoRemove" -Value "1" -Type DWORD
...
function Uninstall-ADTDeployment
{
    ...
    Uninstall-ADTApplication -Name "AzureCopy" -ApplicationType 'MSI'
    ...
    #Cleanup
    Remove-ADTFolder -Path "$envProgramData\AZCopy"
    Remove-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\AZCopy" -Recurse
