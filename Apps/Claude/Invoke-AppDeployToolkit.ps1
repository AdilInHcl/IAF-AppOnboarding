...
$adtSession = @{
    ...
    AppProcessesToClose = @('Claude')  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    $version = Get-Appxpackage -Allusers -Name Claude | Select-Object -ExpandProperty Version
    if(([version]$version -lt [version]$adtSession.AppVersion) -and ($null -ne $version)){	
        Get-AppxPackage "Claude" -AllUsers | Where-Object { $_.Version -ne $adtSession.AppVersion } | Remove-AppxPackage

        Write-ADTLogEntry -Message "Installed Version $($version) is less than packaged version. Hence, proceeding with Claude app upgrade."
    }	
	...
	{
        if($null -eq $version){
        Write-ADTLogEntry -Message "No version of Claude installed in the device. Hence, proceeding with Claude application installation."
        }	
		...
		$msix = "$($adtSession.DirFiles)\Claude.msix"
        $cert = Get-AuthenticodeSignature $msix

        $cert.SignerCertificate | Export-Certificate -FilePath "$env:SystemDrive\Temp\Claudemsix.cer"
        
        Import-Certificate -FilePath "$env:SystemDrive\Temp\Claudemsix.cer"  -CertStoreLocation "Cert:\LocalMachine\Root"        
        
        Import-Certificate -FilePath "$env:SystemDrive\Temp\Claudemsix.cer"  -CertStoreLocation "Cert:\LocalMachine\TrustedPeople"

        Update-ADTDesktop
        
        Start-Sleep -Seconds 20
        
        Add-AppxProvisionedPackage -Online -PackagePath "$($adtSession.DirFiles)\Claude.msix" -SkipLicense

        Write-ADTLogEntry -Message "Claude installed Successfully"
    ...
    if(([version]$version -eq [version]$adtSession.AppVersion))
    {
        ...
        Remove-Item -Path "$env:SystemDrive\Temp\Claudemsix.cer" -Force -ErrorAction SilentlyContinue
...
function Uninstall-ADTDeployment
{
    ...
    Set-NonRemovableAppsPolicy -Online -PackageFamilyName Claude_pzs8sxrjxfjjc -NonRemovable 0 -ErrorAction SilentlyContinue
    
	$test = Get-AppxPackage -name "Claude"
    foreach ($app in $test)
	{
