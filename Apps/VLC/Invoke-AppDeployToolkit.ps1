...
function Install-ADTDeployment
{
    ...
    Uninstall-ADTApplication -Name "VLC" -ApplicationType 'EXE' -ArgumentList "/S /norestart"
    Uninstall-ADTApplication -Name "VLC" -ApplicationType 'MSI'
    Start-Sleep -Seconds 30

    #Cleanup
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\343_VideoLAN_VLCMediaPlayer_3.0.18_PKG_R1'
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\413_VideoLAN_VLCMediaPlayer_3.0.20_PKG_R01'
    Remove-ADTFileFromUserProfiles -Path "AppData\Roaming\vlc" -Recurse
    ...
    Start-ADTProcess -FilePath "$($adtSession.DirFiles)\$($adtSession.InstallerName)" -ArgumentList "/S"
    ...
    #CleanUp
     Remove-ADTFile -Path "$envCommonDesktop\VLC media player.lnk"
     Remove-ADTFile -Path "$envCommonStartMenuPrograms\VideoLAN\Documentation.lnk"
     Remove-ADTFile -Path "$envCommonStartMenuPrograms\VideoLAN\Release Notes.lnk"
     Remove-ADTFile -Path "$envCommonStartMenuPrograms\VideoLAN\VideoLAN Website.lnk"
     Remove-ADTFile -Path "$envCommonStartMenuPrograms\VideoLAN\VLC media player - reset preferences and cache files.lnk"
     Remove-ADTFile -Path "$envCommonStartMenuPrograms\VideoLAN\VLC media player skinned.lnk"
    #ARP 
     Set-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\VLC media player" -Name "NoModify" -Value "1" -Type DWORD
     Set-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\VLC media player" -Name "NoRemove" -Value "1" -Type DWORD
    
    #Shortcut Reconfig
    Remove-ADTFile -Path "$envCommonDesktop\VLC media player.lnk"
    Get-ChildItem -Path "$envCommonStartMenuPrograms\VideoLAN\VLC" -Recurse -ErrorAction SilentlyContinue | Where-Object {$_.Name -ine "VLC media player.lnk"} | ForEach-Object {Remove-ADTFile -Path $_.FullName}

    $ConfigFile = "$envProgramFiles\VideoLAN\VLC\vlcrc"
    Remove-ADTFile -Path $ConfigFile
    New-Item $ConfigFile -type file -Value "[qt]`r`nqt-updates-notif=0`r`nqt-privacy-ask=0"
    Copy-ADTFileToUserProfiles -Path "$ConfigFile" -Destination "AppData\Roaming\VLC"
...
function Uninstall-ADTDeployment
{
    ...
    Show-ADTInstallationWelcome -CloseProcesses @{Name ="VLC"} -Silent
    ...
    #CleanupActions
    $ObjectPath = "$envProgramFiles\VideoLAN"
    Remove-ADTFolder -Path "$ObjectPath\VLC"
    ...
    $ObjectPath = "$envProgramFilesX86\VideoLAN"
    Remove-ADTFolder -Path "$ObjectPath\VLC"
    If((Test-Path -Path $ObjectPath) -and ((Get-ChildItem $ObjectPath -ErrorAction SilentlyContinue | Measure-Object).Count -eq 0)) {Remove-ADTFolder -Path $ObjectPath}
