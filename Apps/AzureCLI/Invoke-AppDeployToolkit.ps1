...
function Install-ADTDeployment
{
    ...
    Uninstall-ADTApplication -Name "Microsoft Azure CLI" -ApplicationType 'MSI'
            #Removing User env. variable
    Invoke-ADTAllUsersRegistryAction {
        Remove-ADTRegistryKey -Key 'HKCU\Environment' -Name 'HTTPS_PROXY' -SID $_.SID
    }
	
	#Cleanup
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\324_Microsoft_AzureCLI_2.45.0_PKG_R1'
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\1275_MS_MicrosoftAzureCLI_2.70.0_PKG_R1'
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AllianzPackages\1275_MS_MicrosoftAzureCLI_2.70.0_PKG_R2'
    ...
    $ARPs = Get-ADTApplication -Name 'Microsoft Azure CLI (64-bit)'
    ...
    }else{Write-ADTLogEntry -Message "Azure CLI app not found, Skipping Branding creation"}
...
function Uninstall-ADTDeployment
{
    ...
    $ARPs = Get-ADTApplication -Name 'Microsoft Azure CLI (64-bit)'
    if (Compare-Version $($adtSession.AppVersion) $ARPs.DisplayVersion "eq") {Write-ADTLogEntry -Message "Azure CLI app found, Skipping Branding deletion"}
