...
$adtSession = @{
    ...
    AppProcessesToClose = @('0install-win','0store-service','DeepL')  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    if(Get-ADTApplication -Name "DeepL" -ErrorAction SilentlyContinue)
    {
        if(Test-Path "$envProgramFiles\Zero Install\0install-win.exe")
        {
            Start-ADTProcess -FilePath "$envProgramFiles\Zero Install\0install-win.exe" -ArgumentList "remove https://appdownload.deepl.com/windows/0install/deepl.xml --machine --batch --background"
        }

        Start-Sleep -s 10
        
        Remove-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\DeepL SE" -Recurse -ErrorAction SilentlyContinue
        
        if(Get-Service -Name "0store-service" -ErrorAction SilentlyContinue)
        {
            sc.exe delete "0store-service"
        }

        if(Test-Path "$envProgramFiles\Zero Install")
        {
            Remove-ADTFolder -Path "$envProgramFiles\Zero Install" -ErrorAction SilentlyContinue
        }

        if(Test-Path "$envProgramData\0install.net")
        {
            Remove-ADTFile -Path "$envProgramData\0install.net" -ErrorAction SilentlyContinue
        }

        Remove-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\https%3a##appdownload.deepl.com#windows#0install#deepl.xml" -ErrorAction SilentlyContinue
        ...
        ForEach ($Path in $ProfilePaths)
        {
            if($Path -ne $public)
            {
                if(Test-Path "$Path\AppData\Roaming\DeepL_SE\settings.json")
                {
                    Remove-ADTFile -Path "$Path\AppData\Roaming\DeepL_SE\settings.json" -ErrorAction SilentlyContinue
                }
            }
        }
    }

    #Cleanup old branding registry
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\519_SE_DeepL_24.2.2.12016_PKG_R1' 
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\519_SE_DeepL_24.2.2.12016_PKG_R2'
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AllianzPackages\1396_DeepL_EnterpriseDesktopApp_25.5.1.16026_PKG_R1' 
    ...
    Start-ADTProcess -FilePath "$($adtSession.DirFiles)\$($adtSession.InstallerName)" -ArgumentList "-S --machine"

    if(Get-ADTApplication -Name "DeepL")
    {
        #Hiding Remove\Change button in AddRemovePrograms
        $ARPs = Get-ADTApplication -Name "DeepL"
        ForEach ($ARP in $ARPs) {
            If ($($ARP.PSPath)) 
            {
        ...
        Set-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\DeepL SE" -Name "proxy.active" -Value "true" -Type String
        Set-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\DeepL SE" -Name "proxy.host" -Value "127.0.0.1" -Type String
        Set-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\DeepL SE" -Name "proxy.port" -Value "9000" -Type String
        Set-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\DeepL SE" -Name "proxy.activecredentials" -Value "false" -Type String

        Remove-ADTFile -Path "$envCommonDesktop\DeepL.lnk" -ErrorAction SilentlyContinue
    }
