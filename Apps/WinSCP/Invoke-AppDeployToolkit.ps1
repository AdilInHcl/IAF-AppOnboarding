...
$adtSession = @{
    ...
    AppProcessesToClose = @('WinSCP')  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    Uninstall-ADTApplication -Name "WinSCP" -ApplicationType 'EXE' -ArgumentList "/VERYSILENT /NORESTART /LOG=""$((Get-ADTConfig).Toolkit.LogPath)\$($adtSession.AppName)_EXE_Uninstall.log"""
    Uninstall-ADTApplication -Name "WinSCP" -ApplicationType 'MSI'

    #Cleanup
    Remove-ADTFolder -Path "$envProgramFilesX86\WinSCP"
    Remove-ADTFolder -Path "$envCommonStartMenu\WinSCP"
    Remove-ADTFolder -Path "$envCommonStartMenuPrograms\WinSCP"
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\586_MartinPrinkryl_WinSCP_6.3.3_PKG_R1'
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\837_MartinPrinkryl_WinSCP_6.3.5_PKG_R1'
...
function Uninstall-ADTDeployment
{
    ...
    Show-ADTInstallationWelcome -CloseProcesses @{Name ="WinSCP"} -Silent
    ...
    $ObjectPath = "HKLM:SOFTWARE\Martin Prikryl"
    Remove-ADTRegistryKey -Key "$ObjectPath\WinSCP 2" -Recurse 
    If((Test-Path -Path $ObjectPath) -and ((Get-ChildItem $ObjectPath -ErrorAction SilentlyContinue | Measure-Object).Count -eq 0)) {Remove-ADTRegistryKey -Key $ObjectPath}

    $ObjectPath = "HKLM:SOFTWARE\WOW6432Node\Martin Prikryl"
    Remove-ADTRegistryKey -Key "$ObjectPath\WinSCP 2" -Recurse 
    Remove-ADTRegistryKey -Key "$ObjectPath\WinSCPCapabilities" -Recurse 
    If((Test-Path -Path $ObjectPath) -and ((Get-ChildItem $ObjectPath -ErrorAction SilentlyContinue | Measure-Object).Count -eq 0)) {Remove-ADTRegistryKey -Key $ObjectPath}
