...
if ($InstalledApp) {
    ...
    if ([version]$InstalledAppVersion -ge [version]$Version) {
        Write-Output "$AppxName is installed and version $InstalledAppVersion is compliant."
    }
    else {
        Write-Output "$AppxName is installed with lower version $InstallAppVersion which is not compliant. Upgrading it to complaint $InstallAppVersion version."
        Exit 1
    }
}
else {
    Write-Output "$AppxName is not installed."
    Exit 1
}
