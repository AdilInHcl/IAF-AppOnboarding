...
$adtSession = @{
    ...
    AppProcessesToClose = @('Konnekt')  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    Uninstall-ADTApplication -Name "Konnekt" -NameMatch 'Exact' -ApplicationType 'MSI'

    #Cleanup
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\908_Glueckkanja_Konnekt_2.9.1_PKG_R1'
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\908_Glueckkanja_Konnekt_2.9.1_PKG_R2'
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\908_Glueckkanja_Konnekt_2.9.1_PKG_R3'
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AllianzPackages\1032_Glueckkanja_Konnekt_2.10.2_PKG_R1'
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AllianzPackages\2021_Glueckkanja_Konnekt_2.11.2_PKG_R1'
    ...
    Start-ADTMsiProcess -Action 'Install' -FilePath "$($adtSession.DirFiles)\$($adtSession.InstallerName)" -AdditionalArgumentList "LAUNCHPROGRAM=0"
    ...
    $ARPs = Get-ADTApplication -Name 'Konnekt'
    ...
    }else{Write-ADTLogEntry -Message "Konnekt app not found, Skipping Branding creation"}
...
function Uninstall-ADTDeployment
{
    ...
    Show-ADTInstallationWelcome -CloseProcesses @{Name ="Konnekt"} -Silent
    ...
    $ARPs = Get-ADTApplication -Name 'Konnekt'
    if (Compare-Version $($adtSession.AppVersion) $ARPs.DisplayVersion "eq") {Write-ADTLogEntry -Message "Konnekt app found, Skipping Branding deletion"}
