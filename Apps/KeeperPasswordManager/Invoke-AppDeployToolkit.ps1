...
$adtSession = @{
    ...
    AppProcessesToClose = @('keeperpasswordmanager')  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
	...
	##============================
	## Uninstalling keeper password Manager
	##============================
	 Uninstall-ADTApplication -Name "Keeper Password Manager" -ArgumentList "/qn"

     $version = Get-Appxpackage -Allusers -Name *KeeperPasswordManager* | Select-Object -ExpandProperty Version
    if(($version -lt $adtSession.AppVersion))
    {
       Set-NonRemovableAppsPolicy -Online -PackageFamilyName KeeperSecurityInc.KeeperPasswordManager_zwgt23r867e2p -NonRemovable 0 -ErrorAction SilentlyContinue

       Set-ADTRegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\Appx" -Name "AllowAllTrustedApps" -Value "1" -Type DWORD -ErrorAction SilentlyContinue
           
       Get-AppxPackage "*KeeperPasswordManager*" -AllUsers -ErrorAction SilentlyContinue | Remove-AppxPackage -AllUsers -ErrorAction SilentlyContinue
    ...
    #Cleanup
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\0292_Keeper_DesktopApp_16.10.1_EN_R01' -ErrorAction SilentlyContinue

    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\366_Keeper_DesktopApp_16.10.5_PKG_R1' -ErrorAction SilentlyContinue

    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\594_Keeper_DesktopApp_16.11.0_PKG_R1' -ErrorAction SilentlyContinue
    
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AllianzPackages\1307_KeeperSecurity_PasswordManager_17.1.1_PKG_R1' -ErrorAction SilentlyContinue
    ...
    Add-AppxProvisionedPackage -Online -PackagePath "$($adtSession.DirFiles)\$($adtSession.InstallerName)" -SkipLicense
...
function Uninstall-ADTDeployment
{
    ...
    $test = Get-AppxPackage -name "*KeeperPasswordManager*"
    foreach ($app in $test)
	{
