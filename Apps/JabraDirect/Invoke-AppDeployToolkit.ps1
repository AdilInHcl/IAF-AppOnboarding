...
$adtSession = @{
    ...
    AppProcessesToClose = @('jabra-direct')  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    Uninstall-ADTApplication -Name "Jabra Direct" -ApplicationType 'EXE' -ArgumentList "/uninstall /quiet /norestart"

    #Cleanup
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\483_GNAudio_JabraDirect_6.13.1801_PKG_R1'
    Remove-ADTRegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\AMC\Packages\991_GNAudio_JabraDirect_6.19.28801_PKG_R1'
    ...
    Start-ADTProcess -FilePath "$($adtSession.DirFiles)\$($adtSession.InstallerName)" -ArgumentList "/install /quiet /norestart" -WindowStyle "Hidden"    
    ...
    If($Platform -eq "RAPPS"){
        ...
        Copy-ADTFileToUserProfiles -Path "$($adtSession.DirFiles)\jabradirectconfig.json" -Destination "AppData\Roaming\Jabra Direct\" -Recurse
    ...
    }else{Write-ADTLogEntry -Message "Jabra Direct app not found, Skipping Branding creation"}
...
function Uninstall-ADTDeployment
{
    ...
    Show-ADTInstallationWelcome -CloseProcesses @{Name ="jabra-direct"} -Silent
